...
TEST(StringsUtil, SplitBug2) {
  EXPECT_EQ(std::vector<std::string>({"set.int32"}), Split("set.int32"));
}

TEST(StringsUtil, SplitWithQuotedElement) {
  EXPECT_EQ(std::vector<std::string>({"a", "b", "c d"}), Split("a b \"c d\""));
}

TEST(StringsUtil, SplitWithInnerQuotes) {
  EXPECT_EQ(std::vector<std::string>({"a", "bc d", "e"}),
            Split("a b\"c d\" e"));
}
